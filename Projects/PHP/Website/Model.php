...
class model
{
	...
	{							// server   usern.  pass db_name
		$this->conn=new Mysqli('localhost','root','','furnishing_php');
	}
	function select($tbl)
	{
		...
		while ($fetch=$res->fetch_object())	//fetch all data from dbase_add_record
		{
			$arr[]=$fetch;
		}
		if(!empty ($arr))
	...
	//$arr=array("name"=>"Ankita","email"=>"ankita@gmail.com","password"=>"7405")
	
	function insert($tbl,$arr)
	{
		$key_arr=array_keys($arr);	//key find arr array("0"=>"name","1"=>"email","2"=>"password")
		$col_str=implode(",",$key_arr);	//array to string (name,email,password)
		
		$value_arr=array_values($arr);
		$value_str=implode("','",$value_arr);	//('Ankita','ankita@gmail.com','7405')
